# ------------------------------------------------------------------------
# VENDORED COPY of the repo-root temperature.py, on purpose: ros2_ws/ builds
# into a container and must never import from the repo root. Keep the two in
# sync by hand (`diff temperature.py ros2_ws/.../drivers/tclab.py` -- only this
# banner and the import line in the docstring should differ).
# temperature_node.py exposes EVERY public method below over one ROS service,
# so extending this class is all it takes to give clients a new capability.
# ------------------------------------------------------------------------
"""
tclab.py - One clean class to drive the Wavelength Electronics TC LAB
...
Quick start:
    from scopio_microscope.drivers.tclab import TCLab
